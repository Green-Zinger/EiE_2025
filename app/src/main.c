...
BT_DATA_BYTES(BT_DATA_FLAGS, flags);
...
int main(void){
  int enable_error = bt_enable(NULL);
  int adv_error = bt_le_adv_start(BT_LE_ADV_CONN_FAST_1, ble_advertising_data, ARRAY_SIZE(ble_advertising_data), NULL, 0);
  if(0!=enable_error){
    kprint("Error code: %d", enable_error)
    return 0;
  }
      
  if(0!=adv_error){
    kprint("Error code %d", adv_error)
    return 0;
  }
